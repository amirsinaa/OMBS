<template>
  <div class="row justify-content-center">
    <input v-model="product.id" type="hidden" id="id">
    ...
    <div class="mt-4 form-group">
      <button @click="productEdit"  type="button" class="btn btn-primary btn-lg btn-block"> {{ $t('c.Send the form')}}</button>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    props: ['product_json','workuri', 'currency_list_json'],
    data() {
      return {
        product: JSON.parse(this.product_json),
      ...
      methods: {
        testData(){
          console.log("uploaded_images:"+this.$store.getters.uploaded_images.join(",")+  ", time: "+ this.product.timelimit.substr(0, 10));
        },
        productEdit(){
          console.log(this.product.title +", " + this.product.description +", timelimit"+ this.product.timelimit+"images_str:"+ this.$store.getters.uploaded_images.join(",") );

          axios.post(this.workuri+'/product/edit/'+this.product.id,{
            id: this.product.id,
            ...
            older_images_str: this.$store.getters.older_uploaded_images.join(","),
          }).then(response => {
            ...
            if(response.data.substr(0,2)=="OK"){
              ...
              $('.message_ok').append('<a href="/dashboard">Back</a>');
